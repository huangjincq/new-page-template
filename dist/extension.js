...
var getPath = (str) => path.resolve(__dirname, str);
var allFeatures = ["batch", "edit", "detail", "delete", "export"];
var allFormColumns = ["accountNumber", "security", "status", "date", "dateRange", "digit", "numberRange"];
function activate(context) {
  const openWebview = vscode.commands.registerCommand("createTemplate.create", (uri) => {
      ...
      async (message) => {
        switch (message.command) {
          case "submit":
            ...
            const templatePath = fs.existsSync(workspaceTemplatePath + "/index.tsx") ? workspaceTemplatePath : getPath(`./templates`);
            const targetPath = uri.fsPath + "/" + pageName;
            await createFiles(templatePath, targetPath, pageConfig);
...
async function createFiles(originalDir, outputDir, pageConfig) {
  ...
  for (const file of files) {
    ...
    if (stats.isDirectory()) {
      ...
      if (file.includes("Modal") && !features.find((feature) => file.toLowerCase().includes(feature))) {
        continue;
      }
      let text = await fs.readFile(filePath, "utf8");
      const exportCode = features.includes("export") ? `frontEndExport={{
         exportFileName: ${"`"}${pageName}_\${moment().format('yyyy-MM-DD')}${"`"},
         permissionUrl: '/pt/${(0, import_lodash.camelCase)(pageName)}/export' 
       }}` : "";
...
function extractComments(text, features, formColumns) {
...
function modifyRouterJs(dir, pageName) {
  ...
  function getInsertPos(fileContent2) {
    return fileContent2.lastIndexOf("]");
  }
  function generateRouteCode(filename, relativePath2) {
    const name = filename.charAt(0).toUpperCase() + filename.slice(1);
    const route = `  {
    name: '${(0, import_lodash.startCase)(name)}',
    route: '/pt/${(0, import_lodash.camelCase)(filename)}',
    component: Loadable(() => import('${relativePath2}${filename}'))
  }`;
    return route;
  }
  function findRouterJs(dir2, depth = 0, track = []) {
    const files = fs.readdirSync(dir2);
    const routerFile = files.find((file) => file === "router.ts" || file === "router.tsx");
    if (routerFile) {
      return {
        routerPath: path.join(dir2, routerFile),
        track: "./" + (track?.length ? track.reverse().join("/") + "/" : "")
      };
    }
    const parentDir = path.resolve(dir2, "..");
    if (parentDir !== dir2 && depth < 3) {
      track.push(path.basename(dir2));
      return findRouterJs(parentDir, depth + 1, track);
    }
  }
}
var generateTableColumns = (tableColumns) => {
  return tableColumns.map((item) => {
    let dataIndex = (0, import_lodash.camelCase)(item);
    ...
    let title = (0, import_lodash.startCase)((0, import_lodash.lowerCase)(item));
